...
app.post("/api/contact", async (req, res) => {
  const fullName = asTrimmedString(req.body?.fullName, 120);
  const phone = asTrimmedString(req.body?.phone, 80);
  const previousVisit = asTrimmedString(req.body?.previousVisit, 120);
  const visitGoals = asStringArray(req.body?.visitGoals);
  const concerns = asStringArray(req.body?.concerns);
  const painFrequency = asTrimmedString(req.body?.painFrequency, 160);
  const painIntensity = asTrimmedString(req.body?.painIntensity, 240);
  const problem = asTrimmedString(req.body?.problem, 1200);
